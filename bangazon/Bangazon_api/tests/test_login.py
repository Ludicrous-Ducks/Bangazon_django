...
import sys
sys.path.append('../')
from Bangazon_api.views.login_view import Login
...
class TestCaseLogin(TestCase):
    ...
    def test_login_view_with_username_password(self):
        ...
        response = self.client.post(reverse('Bangazon_api:login'), user)
    ...
    def test_login_view_with_non_customer_username_password(self):
        """
        This is the test for log in view for a non-customer not passing usename and password
        """

        non_customer = {'username': "bbb", "password": 'bbb00000'}
        response = self.client.post(reverse('Bangazon_api:login'), non_customer)
